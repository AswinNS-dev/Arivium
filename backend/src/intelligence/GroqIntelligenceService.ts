import { getRoleDomain } from '../ai/domain/RoleDomainRegistry';
import { ResourceRecommendation, ResourceRequest } from './types';
...
class GroqIntelligenceService {
  private async complete(system: string, prompt: string, json = false): Promise<string> {
    const apiKey = process.env.GROQ_API_KEY;
    if (!apiKey) throw Object.assign(new Error('GROQ_API_KEY is not configured'), { statusCode: 503 });
    ...
    if (!response.ok) throw Object.assign(new Error(payload?.error?.message || `Groq request failed (${response.status})`), { statusCode: 502 });
    const content = payload?.choices?.[0]?.message?.content;
    if (!content) throw Object.assign(new Error('Groq returned an empty response'), { statusCode: 502 });
    return content;
  }

  async resources(input: ResourceRequest) {
    const domain = getRoleDomain(input.role);
    ...
    const resources: ResourceRecommendation[] = result.resources.map((item: any) => {
      if (!expected.has(item.type) || typeof item.title !== 'string' || typeof item.url !== 'string') throw new Error('Invalid resource item');
      const url = new URL(item.url);
      if (url.protocol !== 'https:') throw new Error('Resource URLs must use HTTPS');
      expected.delete(item.type);
      return { type: item.type, title: item.title, url: url.toString(), reason: String(item.reason || '') };
    });
    if (expected.size) throw Object.assign(new Error('Groq omitted a required resource type'), { statusCode: 502 });
  ...
  async challenges(context: any) {
    const domain = getRoleDomain(context.role);
    ...
    const result = parseJson(await this.complete(system, prompt, true));
    if (!Array.isArray(result?.challenges) || !result.challenges.length) throw Object.assign(new Error('Groq returned invalid challenges'), { statusCode: 502 });
    return result;
  }

  async coach(context: any, question: string) {
...
export default new GroqIntelligenceService();
